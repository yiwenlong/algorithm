...
func setAroundBlank(board [][]byte, position []int) {
	...
	// drill down
	viewAround(board, position, func(i, j int) {
		setAroundBlank(board, []int{i, j})
	})
	// reset
}

func mineAroundCount(board [][]byte, position []int) int {
	count := 0
	viewAround(board, position, func(i, j int) {
		if board[i][j] == 'M' {
			count++
...
func main() {
	...
	fmt.Printf("%v\n", reflect.DeepEqual(board, dstBoard))
}
